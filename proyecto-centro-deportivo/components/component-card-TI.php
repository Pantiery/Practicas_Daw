<section class="hero-section py-5">
  <div class="container">
    <div class="row align-items-center g-4">

      <!-- Contenido -->
      <div class="col-12 col-md-6">
        <h1 class="hero-title">Cafetería</h1>
        <p class="hero-text mt-3">
          Lorem ipsum dolor sit amet et delectus accommodans his consul copiosae legendos at vix
          ad putent delectus delicata usu. Vidit dissentiet eos cu eum an brute copiosae hendrerit.
        </p>
        <div class="d-flex gap-2 mt-4">
          <button class="btn btn-primary">Botón</button>
          <button class="btn btn-outline-dark">Botón</button>
        </div>
      </div>

      <!-- Imagen -->
      <div class="col-12 col-md-6">
        <div class="hero-img-wrapper">
          <img src="../assets/img/Cafeteria_provisional.jpeg" alt="Cafetería" class="img-fluid rounded">
        </div>
      </div>

    </div>
  </div>
</section>

<div class="d-flex justify-content-center">
  <hr class="w-50">
</div>

<section class="hero-section py-5">
  <div class="container">
    <div class="row align-items-center g-4">

      <!-- Imagen -->
      <div class="col-12 col-md-6 order-1 order-md-0">
        <div class="hero-img-wrapper">
          <img src="../assets/img/Salon_de_actos_provisional.jpg" alt="Salón de actos" class="img-fluid rounded">
        </div>
      </div>

      <!-- Contenido -->
      <div class="col-12 col-md-6 order-0 order-md-1">
        <h1 class="hero-title">Salón de actos</h1>
        <p class="hero-text mt-3">
          Lorem ipsum dolor sit amet et delectus accommodans his consul copiosae legendos at vix
          ad putent delectus delicata usu. Vidit dissentiet eos cu eum an brute copiosae hendrerit.
        </p>
        <div class="d-flex gap-2 mt-4">
          <button class="btn btn-primary">Botón</button>
          <button class="btn btn-outline-dark">Botón</button>
        </div>
      </div>

    </div>
  </div>
</section>

<div class="d-flex justify-content-center">
  <hr class="w-50">
</div>

<section class="hero-section py-5">
  <div class="container">
    <div class="row align-items-center g-4">

      <!-- Contenido -->
      <div class="col-12 col-md-6">
        <h1 class="hero-title">Sala de juntas</h1>
        <p class="hero-text mt-3">
          Lorem ipsum dolor sit amet et delectus accommodans his consul copiosae legendos at vix
          ad putent delectus delicata usu. Vidit dissentiet eos cu eum an brute copiosae hendrerit.
        </p>
        <div class="d-flex gap-2 mt-4">
          <button class="btn btn-primary">Botón</button>
          <button class="btn btn-outline-dark">Botón</button>
        </div>
      </div>

      <!-- Imagen -->
      <div class="col-12 col-md-6">
        <div class="hero-img-wrapper">
          <img src="../assets/img/Sala_de_juntas_provisional.jpg" alt="Sala de juntas" class="img-fluid rounded">
        </div>
      </div>

    </div>
  </div>
</section>

<div class="d-flex justify-content-center">
  <hr class="w-50">
</div>

<section class="hero-section py-5">
  <div class="container">
    <div class="row align-items-center g-4">

      <!-- Imagen -->
      <div class="col-12 col-md-6 order-1 order-md-0">
        <div class="hero-img-wrapper">
          <img src="../assets/img/Barbacoa_provisional.jpg" alt="Barbacoa" class="img-fluid rounded">
        </div>
      </div>

      <!-- Contenido -->
      <div class="col-12 col-md-6 order-0 order-md-1">
        <h1 class="hero-title">Barbacoa</h1>
        <p class="hero-text mt-3">
          Lorem ipsum dolor sit amet et delectus accommodans his consul copiosae legendos at vix
          ad putent delectus delicata usu. Vidit dissentiet eos cu eum an brute copiosae hendrerit.
        </p>
        <div class="d-flex gap-2 mt-4">
          <button class="btn btn-primary">Botón</button>
          <button class="btn btn-outline-dark">Botón</button>
        </div>
      </div>

    </div>
  </div>
</section>